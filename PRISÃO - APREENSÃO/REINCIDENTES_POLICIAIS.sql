/*-- ------------------------------------------------------------------------------------------------------------------------------
 * Este código SQL é projetado para identificar indivíduos com indícios de reincidência criminal, por meio da análise de
 * registros de ocorrências policiais no estado de Minas Gerais, no período especificado. A consulta realiza 
 * junções entre as tabelas de ocorrências e de envolvidos, considerando apenas registros fechados, de envolvimento  
 * sendo autor, coautor ou suspeito e informações pessoais completas.
 *
 * São filtradas apenas naturezas criminais específicas, conforme codificações padronizadas, e associadas aos códigos de 
 * RPM e UEOP , com base no município. 
 *
 * Os resultados exibem o nome completo do envolvido, o nome da mãe, a data de nascimento, o total de ocorrências distintas 
 * vinculadas a ele, e listas consolidadas dos códigos e descrições das naturezas em que esteve envolvido.
 -- ------------------------------------------------------------------------------------------------------------------------------*/
SELECT  -- Seleciona os dados pessoais e consolidados dos envolvidos com base nos critérios definidos
    ...
    COUNT(DISTINCT OCO.numero_ocorrencia) AS REINCIDENCIA,  -- Conta o número distinto de ocorrências vinculadas ao envolvido
    GROUP_CONCAT(DISTINCT OCO.natureza_codigo) AS Naturezas_codigo,  -- Consolida os códigos das naturezas das ocorrências distintas
    GROUP_CONCAT(DISTINCT OCO.natureza_descricao) AS Naturezas_descricao,  -- Consolida as descrições das naturezas das ocorrências distintas
...
CASE WHEN OCO.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		ELSE 'OUTROS' 
	END AS UEOP_2025
FROM db_bisp_reds_reporting.tb_ocorrencia OCO  -- Define a tabela principal de ocorrências como origem dos dados
INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV  -- Realiza junção interna com a tabela de envolvidos
   		 ON ENV.numero_ocorrencia = OCO.numero_ocorrencia  -- Estabelece a relação entre ocorrência e envolvido
WHERE 1=1
AND OCO.digitador_id_orgao IN (0,1) -- Registro feito por órgãos específicos(PM , PC)	
AND (ENV.nome_completo_envolvido IS NOT NULL 
		AND ENV.nome_mae IS NOT NULL 
		AND ENV.data_nascimento IS NOT NULL)  -- Garante que os dados pessoais essenciais estejam completos
...
AND ENV.envolvimento_codigo IN ('0100', '0200', '1100')  -- Filtra os tipos de envolvimento: autor, coautor ou suspeito
AND OCO.natureza_codigo IN ('B01121','B01148','B01149','B01147','C01155','C01157','C01159','D01213','D01217','E01250','I04033')  -- Filtra apenas ocorrências com naturezas criminais específicas
...
GROUP BY  -- Agrupa os dados por indivíduo para consolidação das ocorrências
    ENV.nome_completo_envolvido,  -- Agrupa pelo nome completo
    ENV.nome_mae,  -- Agrupa pelo nome da mãe
    ENV.data_nascimento,  -- Agrupa pela data de nascimento
	RPM_2025,
	UEOP_2025
HAVING (COUNT(DISTINCT OCO.numero_ocorrencia) > 1)  -- Retorna apenas indivíduos com mais de uma ocorrência distinta (reincidência)
