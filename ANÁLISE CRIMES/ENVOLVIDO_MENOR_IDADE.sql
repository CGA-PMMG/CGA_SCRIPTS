...
SELECT  
    ...
    CAST(OCO.codigo_municipio AS INTEGER) AS codigo_municipio, -- Converte o código do município para o tipo inteiro.
...
CASE WHEN OCO.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
    ...
    OCO.tipo_logradouro_descricao, -- Descrição do tipo de logradouro onde ocorreu o fato.
    OCO.logradouro_nome, -- Nome do logradouro onde ocorreu o fato.
    OCO.numero_endereco, -- Número do endereço onde ocorreu o fato.
    OCO.nome_bairro, -- Nome do bairro onde ocorreu o fato.
	OCO.unidade_area_militar_codigo, -- Código da unidade área militar.
    OCO.unidade_area_militar_nome, -- Nome da  unidade área milita.
    ...
    OCO.numero_latitude, -- Coordenada de latitude do local da ocorrência.
    OCO.numero_longitude, -- Coordenada de longitude do local da ocorrência.
	OCO.nome_tipo_relatorio, -- Tipo de relatório associado à ocorrência.
    OCO.digitador_sigla_orgao -- Sigla do órgão responsável pelo registro.
FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Tabela principal contendo os registros de ocorrências.
INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV ON OCO.numero_ocorrencia = ENV.numero_ocorrencia -- Faz a junção com a tabela de envolvidos utilizando o número da ocorrência como chave.
WHERE 1=1 -- Condição inicial para facilitar adição de filtros subsequentes.
    ...
    AND OCO.ind_estado IN ('F') -- Filtra ocorrências finalizadas.
ORDER BY RPM_2024, UEOP_2024, OCO.numero_ocorrencia; -- Ordena os resultados por RPM, UEOP e número da ocorrência.