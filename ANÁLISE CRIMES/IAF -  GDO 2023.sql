/*------------------------------------------------------------------------------------------------------------------
 * Indicador de arma de fogo  - GDO 2023
 -----------------------------------------------------------------------------------------------------------------*/
WITH 
 -- Define uma CTE (Common Table Expression) chamada FILTRO para armazenar ocorrências filtradas de acordo com critérios específicos.
FILTRO AS (
SELECT OCO.numero_ocorrencia -- Seleciona o número da ocorrência
FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Especifica a tabela origem como tb_ocorrencia do banco db_bisp_reds_reporting.
WHERE 1=1 -- Mantém a estrutura para facilitar a inclusão de múltiplas condições de filtro.
    AND OCO.ocorrencia_uf = 'MG' -- Filtra ocorrências registradas no estado de Minas Gerais.
    AND OCO.digitador_sigla_orgao IN ('PM', 'PC') -- Inclui apenas registros feitos por órgãos como Polícia Militar (PM) ou Polícia Civil (PC).
    AND OCO.nome_tipo_relatorio IN ('POLICIAL', 'REFAP') -- Considera somente relatórios dos tipos 'POLICIAL' ou 'REFAP'.
    AND OCO.data_hora_fato >= '2025-01-01 00:00:00.000' -- Limita a seleção a ocorrências registradas a partir de 1º de janeiro de 2025.
    AND OCO.ind_estado = 'F' -- Filtra registros fechados.
),
 -- Define uma CTE chamada CRIMES_AF para calcular a quantidade de crimes específicos associados às ocorrências filtradas.
CRIMES_AF AS (
SELECT OCO.numero_ocorrencia, -- Seleciona o número da ocorrência como identificador principal.
       COUNT(*) AS QTDE_CAF -- Conta o número total de registros que atendem aos critérios.
FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Especifica a tabela origem como tb_ocorrencia.
INNER JOIN FILTRO F ON OCO.numero_ocorrencia = F.numero_ocorrencia -- Realiza um INNER JOIN com a CTE FILTRO para considerar apenas as ocorrências previamente filtradas.
WHERE 1=1 -- Mantém a estrutura para facilitar a adição de condições adicionais.
    AND OCO.natureza_codigo IN ( -- Filtra registros que possuem códigos de natureza específicos.
            'B01121', -- HOMICÍDIO
            'C01157', -- ROUBO
            'C01158', -- EXTORSÃO
            'C01159', -- EXTORSÃO MEDIANTE SEQÜESTRO
            'D01213', -- ESTUPRO
            'D01217', -- ESTUPRO DE VULNERÁVEL
            'E03015', --  DISPARO DE ARMA DE FOGO OU ACIONA...
            'B02001', -- TORTURA
            'B01504' -- FEMINICÍDIO 
        )
GROUP BY 1 -- Agrupa os resultados pelo número de ocorrência para calcular corretamente a quantidade de registros por ocorrência.
),
-- Define uma CTE chamada PRISOES para calcular informações sobre prisões associadas às ocorrências filtradas.
PRISOES AS ( 
    SELECT OCO.numero_ocorrencia, -- Seleciona o número da ocorrência
           SUM(CASE -- Calcula o total de prisões classificadas 
                WHEN 
                    OCO.natureza_codigo IN (
                        'B01121', 'B01148', 'C01157', 'C01158', 'C01159', 
                        'D01213', 'D01217', 'B02001', 'B01504' 
                    )  -- Verifica se o código da natureza da ocorrência  corresponde a HOMICÍDIO, SEQÜESTRO OU CÁRCERE PRIVADO, ROUBO, EXTORSÃO,EXTORSÃO MEDIANTE SEQÜESTRO,ESTUPRO,ESTUPRO DE VULNERÁVEL,
					-- TORTURA,FEMINICÍDIO 
                    OR (OCO.natureza_codigo = 'A05000' AND ENV.natureza_ocorrencia_codigo IN ( 
                        'B01121', 'B01148', 'C01157', 'C01158', 'C01159', 
                        'D01213', 'D01217', 'B02001', 'B01504'
                    ))  -- Verifica se o código da natureza da ocorrência corresponde a AVERIGUAÇÃO DE PESSOA / VEÍCULO EM ATITUDE SUSPEITA 
                    -- e a natureza do envolvido a HOMICÍDIO, SEQÜESTRO OU CÁRCERE PRIVADO, ROUBO, EXTORSÃO,EXTORSÃO MEDIANTE SEQÜESTRO,
                    --ESTUPRO,ESTUPRO DE VULNERÁVEL,TORTURA,FEMINICÍDIO .
                THEN 1 -- Conta como 1 se a condição for atendida.
                ELSE 0 -- Conta como 0 se a condição não for atendida.
            END) AS CV, -- Totaliza crimes associados à ocorrência.
           SUM(CASE -- Calcula o total de prisões classificadas como crimes comuns (COMUM).
                WHEN 
                    OCO.natureza_codigo IN ( 
                        'B01121', 'B01148', 'C01157', 'C01158', 'C01159', 
                        'D01213', 'D01217', 'B02001', 'B01504' 
                    ) -- Verifica se o código da natureza corresponde a crimes violentos, como homicídio ou sequestro.
                    OR (OCO.natureza_codigo = 'A05000' AND ENV.natureza_ocorrencia_codigo IN ( 
                        'B01121', 'B01148', 'C01157', 'C01158', 'C01159', 
                        'D01213', 'D01217', 'B02001', 'B01504'
                    )) -- Verifica crimes violentos relacionados a averiguações.
                THEN 0 -- Conta como 0 para crimes violentos.
                ELSE 1 -- Conta como 1 para crimes comuns.
            END) AS COMUM, -- Totaliza crimes comuns associados à ocorrência.
           COUNT(*) AS QTDE_PRISOES -- Conta o total de prisões associadas às ocorrências.
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Especifica a tabela origem como tb_ocorrencia.
    INNER JOIN FILTRO F ON OCO.numero_ocorrencia = F.numero_ocorrencia -- Junta os dados da CTE FILTRO com os registros de tb_ocorrencia.
    LEFT JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV ON ENV.numero_ocorrencia = OCO.numero_ocorrencia -- Realiza um LEFT JOIN para incluir informações de envolvidos relacionados às ocorrências.
    WHERE 1=1 -- Mantém a estrutura para facilitar a inclusão de condições adicionais.
        AND OCO.digitador_sigla_orgao = 'PM' -- Limita a seleção a registros feitos pela Polícia Militar.
        AND ENV.id_tipo_prisao_apreensao IN (1,2,3,4,6,7) -- Filtra registros de prisões/apreensões específicas por tipo.
    GROUP BY 1 -- Agrupa os resultados pelo número de ocorrência.
),
 -- Define uma CTE para calcular a quantidade de simulacros de armas apreendidos.
SIMULACROS AS (
    SELECT OCO.numero_ocorrencia, -- Seleciona o número da ocorrência
           COUNT(*) AS QTDE_SIMULACROS -- Conta o número total de simulacros associados às ocorrências.
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Especifica a tabela origem como tb_ocorrencia.
    INNER JOIN PRISOES P ON OCO.numero_ocorrencia = P.numero_ocorrencia -- Junta os dados da CTE PRISOES para incluir apenas ocorrências com prisões.
    LEFT JOIN db_bisp_reds_reporting.tb_material_apreendido_ocorrencia MAT ON MAT.numero_ocorrencia = OCO.numero_ocorrencia -- Junta informações de materiais apreendidos.
    WHERE 1=1 -- Mantém a estrutura para facilitar a inclusão de condições adicionais.
        AND P.CV = 1 -- Filtra apenas ocorrências classificadas como crimes violentos.
        AND OCO.digitador_id_orgao = 0 -- Limita a seleção a registros com digitador_id_orgao igual a 0, PM.
        AND MAT.tipo_objeto_codigo = '2020' -- Filtra apenas objetos do tipo: SIMULACRO DE ARMA DE FOGO (USO RESTRITO).
        AND MAT.situacao_codigo = '0100' -- Considera apenas simulacros na situação: APREENDIDO.
    GROUP BY 1 -- Agrupa os resultados pelo número de ocorrência.
),
-- Define uma CTE para calcular a quantidade de armas de fabricação artesanal apreendidas.
ARTESANAIS AS ( 
    SELECT OCO.numero_ocorrencia, -- Seleciona o número da ocorrência 
           COUNT(*) AS QTDE_ARTESANAIS -- Conta o número total de armas artesanais associadas às ocorrências.
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Especifica a tabela origem como tb_ocorrencia.
    INNER JOIN PRISOES P ON OCO.numero_ocorrencia = P.numero_ocorrencia -- Junta os dados da CTE PRISOES para incluir apenas ocorrências com prisões.
    LEFT JOIN db_bisp_reds_reporting.tb_arma_ocorrencia ARMA ON ARMA.numero_ocorrencia = OCO.numero_ocorrencia -- Junta informações de armas apreendidas.
    WHERE 1=1 -- Mantém a estrutura para facilitar a inclusão de condições adicionais.
        AND P.CV = 1 -- Filtra apenas ocorrências classificadas como crimes violentos.
        AND OCO.digitador_id_orgao = 0 -- Limita a seleção a registros com digitador_id_orgao igual a 0, apenas PM.
        AND ARMA.marca_arma_descricao = 'FABRICACAO ARTESANAL' -- Filtra apenas armas de fabricação artesanal.
        AND ARMA.situacao_descricao = 'APREENDIDO' -- Considera apenas armas na situação 'APREENDIDO'.
    GROUP BY 1 -- Agrupa os resultados pelo número de ocorrência.
),
-- Define uma CTE para calcular a quantidade de armas industrializadas apreendidas.
INDUSTRIALIZADAS AS ( 
    SELECT OCO.numero_ocorrencia, -- Seleciona o número da ocorrência 
           COUNT(*) AS QTDE_INDUST -- Conta o número total de armas industrializadas associadas às ocorrências.
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Especifica a tabela origem como tb_ocorrencia.
    INNER JOIN PRISOES P ON OCO.numero_ocorrencia = P.numero_ocorrencia -- Junta os dados da CTE PRISOES para incluir apenas ocorrências com prisões.
    INNER JOIN db_bisp_reds_reporting.tb_arma_ocorrencia ARMA ON OCO.numero_ocorrencia = ARMA.numero_ocorrencia -- Junta informações de armas apreendidas.
    WHERE 1=1 -- Mantém a estrutura para facilitar a inclusão de condições adicionais.
        AND OCO.digitador_id_orgao = 0 -- Limita a seleção a registros com digitador_id_orgao igual a 0, apenas PM.
        AND ARMA.marca_arma_descricao <> 'FABRICACAO ARTESANAL' -- Exclui armas de fabricação artesanal.
        AND ARMA.marca_arma_descricao IS NOT NULL -- Filtra apenas armas com descrição de marca não nula.
        AND ARMA.situacao_descricao IN ('APREENDIDO', 'RECUPERADO') -- Considera armas na situação 'APREENDIDO' ou 'RECUPERADO'.
        AND ARMA.tipo_arma_descricao NOT IN ( 
            'ARMA DE PRESSAO IGUAL FZ 7,62MM',
            'ARMAS DE PRESSAO ACIMA DE 6MM',
            'ARMAS DE PRESSAO IGUAL OU INFERIOR A 6MM'
        )-- Exclui tipos específicos de armas de pressão.
        AND ARMA.tipo_arma_descricao IS NOT NULL -- Filtra apenas armas com descrição de tipo não nula.
    GROUP BY 1 -- Agrupa os resultados pelo número de ocorrência.
)
SELECT 
	CASE WHEN OCO.codigo_municipio IN (310620) THEN '01 RPM'
   	...
   	END AS RPM_2025, -- Formula RPM 2024
CASE WHEN OCO.codigo_municipio in (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		ELSE 'OUTROS'
	END AS UEOP_2025, -- Formula Ueop 2024
     OCO.numero_ocorrencia, -- Seleciona o número da ocorrência como identificador principal.
    OCO.natureza_codigo, -- Seleciona o código de natureza associado à ocorrência.
    OCO.natureza_descricao, -- Seleciona a descrição da natureza da ocorrência.
    CAST(OCO.codigo_municipio AS INTEGER) AS codigo_municipio, -- Converte e seleciona o código do município como inteiro.
    OCO.data_hora_fato, -- Seleciona a data e hora do fato.
    OCO.digitador_sigla_orgao, -- Indica o órgão que registrou a ocorrência.
    OCO.unidade_area_militar_codigo, -- Seleciona o código da área militar responsável.
    ...
    OCO.nome_tipo_relatorio, -- Indica o tipo de relatório.
    C.QTDE_CAF AS CAF, -- Seleciona a quantidade de crimes específicos filtrados pela CTE CRIMES_AF.
    P.CV AS PRI_CV, -- Seleciona o total de crimes violentos filtrados pela CTE PRISOES.
    P.COMUM AS PRI_COMUM, -- Seleciona o total de crimes comuns filtrados pela CTE PRISOES.
    P.QTDE_PRISOES AS PRI_TOTAL, -- Seleciona o total de prisões filtradas pela CTE PRISOES.
    S.QTDE_SIMULACROS AS SCV, -- Seleciona a quantidade de simulacros filtrados pela CTE SIMULACROS.
    A.QTDE_ARTESANAIS AS ACV, -- Seleciona a quantidade de armas artesanais filtradas pela CTE ARTESANAIS.
    I.QTDE_INDUST AS AI -- Seleciona a quantidade de armas industrializadas filtradas pela CTE INDUSTRIALIZADAS.
FROM db_bisp_reds_reporting.tb_ocorrencia OCO -- Especifica a tabela origem como tb_ocorrencia.
INNER JOIN FILTRO F ON OCO.numero_ocorrencia = F.numero_ocorrencia -- Junta os dados da CTE FILTRO para Selecionar apenas ocorrências filtradas.
LEFT JOIN CRIMES_AF C ON OCO.numero_ocorrencia = C.numero_ocorrencia -- Junta os dados da CTE CRIMES_AF para Selecionar informações sobre crimes específicos.
LEFT JOIN PRISOES P ON OCO.numero_ocorrencia = P.numero_ocorrencia -- Junta os dados da CTE PRISOES para Selecionar informações sobre prisões.
LEFT JOIN SIMULACROS S ON OCO.numero_ocorrencia = S.numero_ocorrencia -- Junta os dados da CTE SIMULACROS para Selecionar informações sobre simulacros apreendidos.
LEFT JOIN ARTESANAIS A ON OCO.numero_ocorrencia = A.numero_ocorrencia -- Junta os dados da CTE ARTESANAIS para Selecionar informações sobre armas artesanais.
LEFT JOIN INDUSTRIALIZADAS I ON OCO.numero_ocorrencia = I.numero_ocorrencia -- Junta os dados da CTE INDUSTRIALIZADAS para Selecionar informações sobre armas industrializadas.
HAVING -- Filtra os resultados para considerar apenas ocorrências que atendam a pelo menos uma das condições abaixo.
    (C.QTDE_CAF > 0 OR -- Seleciona ocorrências com pelo menos um crime específico registrado.
    S.QTDE_SIMULACROS > 0 OR -- Seleciona ocorrências com pelo menos um simulacro apreendido.
    A.QTDE_ARTESANAIS > 0 OR -- Seleciona ocorrências com pelo menos uma arma artesanal apreendida.
    I.QTDE_INDUST > 0) -- Seleciona ocorrências com pelo menos uma arma industrializada apreendida.
ORDER BY OCO.data_hora_fato ASC; -- Ordena os resultados pela data e hora do fato em ordem crescente.
