/*------------------------------------------------------------------------------------------------------------------------------------------------------
 * Este código SQL é projetado para identificar autores presos pelos crimes do Indicador de Resultado **** MV(Morte Violenta)*** da GDO 2025 com mandado
 * de prisão em aberto.
 * A consulta é realizada nas tabelas de ocorrências e envolvidos, filtrando por naturezas MV(Morte Violenta) e garantindo que os envolvidos
 *  tenham informações pessoais completas.
  
 -----------------------------------------------------------------------------------------------------------------------------------------------------*/

WITH
PRESOS_MP_MV AS  -- Define uma CTE (Common Table Expression) chamada PRESOS_MP_MV - Autores, Co-Autores e Suspeitos de crimes de Morte Violenta
(
	SELECT 
	...
	AND (
		...
		AND ENV.natureza_ocorrencia_codigo IN ('B01121','C01157','B02001','B01129','B01148','B01504') -- Filtra pela natureza específica da ocorrência - MV
        ...
        AND ENV.numero_ocorrencia IN (
						         ...
						         WHERE 1=1 
									...
									AND vit.natureza_ocorrencia_codigo IN ('B01121','C01157','B02001','B01129','B01148','B01504') -- A vítima deve ter a mesma natureza da ocorrência que o autor - MV
...
FROM 
	...
	INNER JOIN PRESOS_MP_MV PMV ON OCO.numero_ocorrencia = PMV.numero_ocorrencia -- Associação das ocorrências com os envolvidos
WHERE 1=1
