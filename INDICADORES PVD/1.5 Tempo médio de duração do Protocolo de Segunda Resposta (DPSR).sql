/* ----------------------------------------------------------------------------------------------------------------------------------------------------
 *  ================================================================================================================================================
 *  ========================================= 1.5 TEMPO MÉDIO DE DURAÇÃO DO PROTOCOLO DE SEGUNDA RESPOSTA (DPSR) ===================================
 *  ================================================================================================================================================
 * 
 * O Indicador tem por finalidade avaliar se o Protocolo de Segunda Resposta está sendo aplicado em tempo adequado.
 *-----------------------------------------------------------------------------------------------------------------------------------------------------*/
WITH Inclusao AS (
    SELECT 
        ENV.nome_completo_envolvido,                                    -- Nome da vítima 
        ENV.data_nascimento,                                            -- Data de nascimento 
        ENV.nome_mae,												    -- Nome da mãe da vítima 
   	...
   	END AS RPM_2025,
CASE
  WHEN OCO.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
	...
	END AS UEOP_2025,	
        OCO.data_hora_fato AS data_Inclusao,                            -- Data/hora de inclusão da ocorrência - Protocolo (natureza A20.003)
        YEAR(OCO.data_hora_fato) AS ano_Inclusao,                       -- Ano do fato
        OCO.numero_ocorrencia -- Número da ocorrência de inclusão
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO
    INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV 
        ON OCO.numero_ocorrencia = ENV.numero_ocorrencia
    WHERE 
        OCO.natureza_codigo = 'A20003'         -- Natureza de inclusão no protocolo
       ...
       AND OCO.nome_tipo_relatorio IN ('POLICIAL', 'BOS', 'BOS AMPLO')    -- Tipos de relatório Polical, Bos e Bos Amplo
...
Encerramento AS (
    SELECT 
        ...
        ENV.nome_mae, 													-- Nome da mãe da vítima 
        OCO.codigo_municipio,                                           -- Código  do município
        OCO.numero_ocorrencia AS numero_ocorrencia_Encerramento,        -- Identificador da ocorrência de encerramento
        OCO.data_hora_fato AS data_Encerramento,                        -- Data/hora de encerramento (naturezas A20.014, A20.020, A20.024)
        OCO.natureza_codigo,
        YEAR(OCO.data_hora_fato) AS ano_Encerramento,                   -- Ano de encerramento
        MONTH(OCO.data_hora_fato) AS mes_Encerramento,                  -- Mês de encerramento
        ROW_NUMBER() OVER (
            PARTITION BY ENV.nome_completo_envolvido, ENV.data_nascimento 
            ORDER BY OCO.data_hora_fato
        ) AS rn                                                         -- Ordena encerramentos para selecionar o primeiro
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO
    INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV 
        ON OCO.numero_ocorrencia = ENV.numero_ocorrencia
    WHERE 
        OCO.natureza_codigo IN ('A20014','A20020','A20024')   -- Naturezas de encerramento
...
InclusaoUnica AS (
    SELECT 
        ...
        I.data_nascimento,              -- Data de nascimento da vítima       
        I.nome_mae,						-- Nome da mãe da vítima                       
        I.codigo_municipio,             -- Código de município da vítima                              
        I.RPM_2025,                        -- RPM  
        I.UEOP_2025,						-- UEOP                         
        I.data_Inclusao,                  -- Data da inclusão da ocorrência (Protocolo)                        
        I.numero_ocorrencia,                -- Número da ocorrência de inclusão                          
        ROW_NUMBER() OVER (
            PARTITION BY I.nome_completo_envolvido, I.data_nascimento 
            ORDER BY I.data_Inclusao
        ) AS row_num,
        I.natureza_codigo
    FROM Inclusao I 
)  -- Esta CTE seleciona a primeira inclusão por vítima
SELECT DISTINCT
	CONCAT(UPPER(I.nome_completo_envolvido),'-',E.numero_ocorrencia_Encerramento) AS chave_nome_bo, -- Concatena o nome completo do envolvido e o número do registro, criando uma chave única para cada um dos envolvidos no registro
    I.nome_completo_envolvido,   -- Nome da vítima    
    I.data_nascimento,              -- Data de nascimento da vítima       
    I.data_Inclusao,    			-- Data da inclusão do protocolo (data do fato)
    E.data_Encerramento,			-- Data do encerramento do protocolo (data do fato)
    DATEDIFF(E.data_Encerramento, I.data_Inclusao) AS dias_duracao_protocolo,  -- Diferença em dias entre inclusão e encerramento
    E.ano_Encerramento,    -- Ano do encerramento do protocolo                                            
    E.mes_Encerramento,    -- Mês do encerramento do protocolo                                              
    I.numero_ocorrencia,      -- Número da ocorrência de inclusão                                        
    E.numero_ocorrencia_Encerramento,     -- Número da ocorrência de encerramento                             
    I.codigo_municipio,
    I.RPM_2025,                           -- RPM da inclusão   
    I.UEOP_2025 ,
    I.natureza_codigo,
    E.natureza_codigo
FROM InclusaoUnica I 
JOIN Encerramento E 
    ON UPPER(I.nome_completo_envolvido) = UPPER(E.nome_completo_envolvido) 
    AND I.data_nascimento = E.data_nascimento 
WHERE 
    I.row_num = 1         -- Apenas a primeira inclusão por vítima
    AND E.rn = 1          -- Apenas o primeiro encerramento correspondente
    AND I.data_Inclusao < E.data_Encerramento  -- Garante ordem cronológica correta, onde a data do encerramento é maior que a data da inclusão
ORDER BY 
    E.ano_Encerramento,                                                 
    E.mes_Encerramento,                                                
    I.nome_completo_envolvido;       -- Ordena por ano, mês e nome da vítima
