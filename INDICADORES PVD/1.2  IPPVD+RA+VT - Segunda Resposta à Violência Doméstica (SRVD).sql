/* ----------------------------------------------------------------------------------------------------------------------------------------------------
 *  ================================================================================================================================================
 *  ====================================== 1.2 IPPVD + RA + VT -> SEGUNDA RESPOSTA À VIOLÊNCIA DOMÉSTICA (SRVD) ====================================
 *  ================================================================================================================================================
 * 
 *  Os itens PPVD sendo o número de inclusões no Protocolo de Segunda Resposta (A20.003), RA o número de recusas de atendimento (A20.005) e o
 *  VT o número de visitas tranquilizadoras (A20.002) correspondem ao numerador do indicador Segunda Resposta à Violência Doméstica (SRVD) que tem 
 *  por finalidade aferir o esforço das Unidades no que tange à abrangência de intervenção para atendimento de segunda resposta.
 *-----------------------------------------------------------------------------------------------------------------------------------------------------*/
SELECT
   OCO.numero_ocorrencia,                                   -- Seleciona o número ocorrência
   UPPER(ENV.nome_completo_envolvido) nome_completo_envolvido,					    -- Seleciona o nome completo do envolvido
   CONCAT(UPPER(ENV.nome_completo_envolvido),'-',OCO.numero_ocorrencia) AS chave_nome_bo, -- Concatena o nome completo do envolvido e o número do registro, criando uma chave única para cada um dos envolvidos no registro
   CASE WHEN OCO.natureza_codigo = 'A20003' THEN 'IPPVD'    	-- Se o código da natureza  A20003 retorna IPPVD
   		WHEN OCO.natureza_codigo = 'A20005' THEN 'RA' 	    	-- Se o código da natureza  A20005 retorna RA
   		WHEN OCO.natureza_codigo = 'A20002'	THEN 'VT'	    	-- Se o código da natureza  A20002 retorna VT
   ELSE 'OUTRO'													-- Se o não especificado retorna OUTRO
   END ATENDIMENTO_PPVD,								    -- Coluna calculada para retornar o tipo de atendimento PPVD
   OCO.natureza_codigo,                                     -- Seleciona o código da natureza da ocorrência
   OCO.natureza_descricao,									-- Seleciona o código da natureza da ocorrência
   ...
   MONTH(OCO.data_hora_fato) AS mes_fato,                   -- Extrai o mês da data do fato e o rotula como “mes_fato”
...
WHERE 1 = 1
    ...
    AND OCO.digitador_sigla_orgao = 'PM'                    -- Inclui somente registros digitados pela Polícia Militar (PM)
    AND OCO.natureza_codigo IN ('A20003', 'A20005','A20002') -- Filtra as naturezas : Protocolo de Segunda Resposta (A20.003), Recusas de atendimento (A20.005) e Visitas tranquilizadoras (A20.002)
