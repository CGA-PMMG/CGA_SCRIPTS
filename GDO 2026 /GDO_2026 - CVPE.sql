...
LETALIDADE AS                                                        -- Define uma tabela temporária para filtrar ocorrências com letalidade policial
( 
    ...
    WHERE 1=1                                                           -- Inicia as condições de filtro
        ...
        AND ENV.orgao_lotacao_policial_sigla = 'PM' 				 -- Filtra sigla do órgão policial, PM
   		AND YEAR(ENV.data_hora_fato) BETWEEN 2020 AND 2024 
)
...
CASE WHEN OCO.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		WHEN OCO.codigo_municipio IN (310100,310270,310340,311700,311950,312235,313330,313400,314140,314630,314870,315217,317160) THEN '70 BPM' -- OK 08/01/2026
		...
		WHEN OCO.codigo_municipio IN (310170,310520,312245,312560,313470,313580,313600,313650,314055,314315,314675,315510,315660,315710,315810,316030) THEN '44 BPM' -- OK 08/01/2026
...
INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia AS ENV ON OCO.numero_ocorrencia = ENV.numero_ocorrencia    -- Join com tabela de envolvidos
