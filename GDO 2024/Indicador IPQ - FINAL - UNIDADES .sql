/*----------------------------------------------------------------------------------------------------------------------------------
 * Este script foi desenvolvido para análise do indicador da GDO, IPQ (Indicador de Prisão de Qualificada) das unidades. 
 * Versão 10/04/2024
 ----------------------------------------------------------------------------------------------------------------------------------*/
WITH 
PRESOS_MV AS 
( ----- ESTA CTE RETORNA PRESOS POR MV ------
    SELECT
        ...
        CAST(ENV.data_nascimento as date) AS 'data_nascimento' -- SELECIONA E CONVERTE A DATA DE NASCIMENTO DO ENVOLVIDO EM DATA
    FROM
        db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV -- TABELA ENVOLVIDOS
    WHERE
        1 = 1
        AND ENV.data_hora_fato >= '2019-01-01' -- FILTRA OCORRÊNCIAS A PARTIR DE 2019
        AND ENV.digitador_id_orgao in (0,1) -- FILTRA ORGÃO DIGITADOR COMO PM OU PC
		AND ENV.id_tipo_prisao_apreensao IN (1,2,3,4,6,7)-- FILTRA FLAGRANTES(REDS E TCO),MANDADO, RECAPTURA, OUTRAS PRISOES
        AND ENV.numero_ocorrencia IN (
            SELECT
                ENV2.numero_ocorrencia  -- SELECIONA O NÚMERO DA OCORRÊNCIA
            FROM
                db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV2 -- TABELA ENVOLVIDO
            WHERE
                1 = 1
                AND ENV2.data_hora_fato >= '2019-01-01' -- FILTRA OCORRÊNCIAS A PARTIR DE 2019
                AND ENV2.digitador_id_orgao in (0,1) -- FILTRA ORGÃO DIGITADOR COMO PM OU PC
                AND ENV2.natureza_ocorrencia_codigo IN ( 'B01121','C01157','B02001','B01129') -- FILTRA HOMICÍDIO, ROUBO, TORTURA E LESÃO CORPORAL
                AND ENV2.condicao_fisica_descricao = 'FATAL' -- FILTRA CONDIÇÃO FÍSICA COMO FATAL
                AND ENV2.id_envolvimento IN (25,32,1097,26,27,28,872) -- CÓDIGOS PARA VÍTIMAS: 25,26,27,28,32,872,1097
        )
),
PRESOS_ROUBO_FURTO AS 
( ----- ESTA CTE RETORNA PRESOS POR CCP -------
    SELECT
        UPPER(ENV.nome_completo_envolvido) as NOME_COMPLETO_ENVOLVIDO, -- SELECIONA O NOME COMPLETO DO ENVOLVIDO
        UPPER(ENV.nome_mae) AS NOME_MAE, -- SELECIONA O NOME DA MÃE DO ENVOLVIDO
        ENV.nome_pai, -- SELECIONA O NOME DO PAI DO ENVOLVIDO
    ...
    WHERE
        ...
        AND ENV.data_hora_fato >= '2019-01-01' -- FILTRA OCORRÊNCIAS A PARTIR DE 2019
        AND ENV.digitador_id_orgao in (0,1) -- FILTRA ORGÃO DIGITADOR COMO PM OU PC
        AND ENV.natureza_ocorrencia_codigo IN ( 'C01157',  'C01155', 'C01158') -- ROUBO, FURTO, EXTORSÃO
        AND ENV.id_tipo_prisao_apreensao IN (1,2,3,4,6,7)	-- FLAGRANTES(REDS E TCO),MANDADO, RECAPTURA, OUTRAS PRISOES
),
PRESOS_VDOM AS 
( ---- ESTA CTE RETORNA PRESOS POR VIOLENCIA DOMESTICA ------
    SELECT
        UPPER(ENV.nome_completo_envolvido) as NOME_COMPLETO_ENVOLVIDO, -- SELECIONA O NOME COMPLETO DO ENVOLVIDO
        UPPER(ENV.nome_mae) AS NOME_MAE, -- SELECIONA O NOME DA MÃE DO ENVOLVIDO
        ENV.nome_pai, -- SELECIONA O NOME DO PAI DO ENVOLVIDO
    ...
    WHERE
        ...
        AND ENV.numero_ocorrencia IN (
            ...
            WHERE
                1 = 1
                AND ENV2.data_hora_fato >= '2019-01-01' -- FILTRA OCORRÊNCIAS A PARTIR DE 2019
                AND ENV2.codigo_sexo ='F' -- FILTRA VITIMA DO SEXO FEMININO
				AND ENV2.id_relacao_vitima_autor IN (22,21,15,6,7,19,18,16,5,20) -- FILTRA A RELAÇÃO VITIMA/AUTOR
				AND ENV2.id_envolvimento IN (25,32,872) -- FILTRA O ID DO ENVOLVMENTO
        )
),
PRESOS_MANDADO AS 
( ------ ESTA CTE RETORNA ENVOLVIDOS COM MANDADO DE PRISÃO EM ABERTO ------
    SELECT
        ...
        ENV.data_hora_fato, -- SELECIONA A DATA/HORA DO FATO
         ROW_NUMBER() OVER (PARTITION BY 
                    ...
                    ORDER BY ENV.data_hora_fato) AS duplicado,  -- ESTA FUNÇÃO NUMERA A SAÍDA DO CONJUTO DO RESULTADO, NUMERANDO DE 1 A N CADA PARTIÇÃO.  
        CASE 
	...
	CASE 
   	...
   	END AS RPM_2024, 
   	CAST(BO.codigo_municipio as integer) AS codigo_municipio,  -- SELECIONA E CONVERTE O CÓDIGO DO MUNCIPIO PARA INTEIRO
   	BO.nome_municipio,  -- SELECIONA O NOME DO MUNICIPIO
   	BO.unidade_area_militar_codigo, -- SELECIONA O CÓDIGO DA ÁREA MILITAR
   	BO.unidade_area_militar_nome, -- SELECIONA O NOME DA ÁREA MILITAR
    ...
    FROM
        db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV  -- TABELA ENVOLVIDO
        INNER JOIN db_bisp_reds_reporting.tb_ocorrencia BO -- TABELA OCORRÊNCIA
        ON BO.numero_ocorrencia = ENV.numero_ocorrencia -- JUNÇÃO PELO NÚMERO DA OCORRÊNCIA
    WHERE
        1 = 1 -- CONDIÇÃO SEMPRE VERDADEIRA PARA FACILITAR A ADIÇÃO DE OUTRAS CONDIÇÕES
        AND YEAR(BO.data_hora_fato) = :ANO -- PROMPT PARA FILTRAR O ANO DA DATA/HORA DO FATO
        AND MONTH (BO.data_hora_fato) >= :MESINICIAL -- PROMPT PARA FILTRAR O MÊS INICIAL DA DATA/HORA DO FATO
        AND MONTH (BO.data_hora_fato) <= :MESFINAL -- PROMPT PARA FILTRAR O MÊS FINAL DA DATA/HORA DO FATO
        AND ENV.digitador_id_orgao in (0,1) -- FILTRA ORGÃO DIGITADOR COMO PM OU PC
        AND (
        ...
        AND BO.digitador_id_orgao  = 0 -- FILTRA ORGÃO DIGITADOR COMO PM 
        AND BO.nome_tipo_relatorio not in ('RAT','BOS') -- EXCLUI TIPO RELATÓRIO RAT E BOS 
        AND BO.ocorrencia_uf = 'MG' -- FILTRA UF DA OCORRÊNCIA, MG
...
--- ESTA SELEÇÃO RETORNA OS PRESOS QUE CONTAM PARA O INDICADOR IPQ ---
SELECT
    DISTINCT
    CASE
        ...
        ELSE 0
    END AS PESO, -- ESTE CASE ATRIBUI UMA VALOR PARA O PESO DE ACORDO COM INDICADOR
    CASE
        ...
        ELSE 'MP_COMUM'
    END AS PESO_DESCRICAO, -- ESTE CASE RETORNA A DESCRÇÃO DO PESO ATRIBUIDO
    YEAR (M.data_hora_fato) as ANO, -- ANO DA DATA/HORA DO FARO DO MADADO DE PRISÃO
    MONTH (M.data_hora_fato) as mes, -- MÊS DA DATA/HORA DO FARO DO MADADO DE PRISÃO
    M.*
FROM
    PRESOS_MANDADO M
       LEFT JOIN PRESOS_MV MV ON (
        ...
        AND M.data_hora_fato > MV.data_hora_fato -- JUNÇÃO COM A TABELA QUE CALCULA OS PRESOS POR MORTE VIOLENTA
    )
    LEFT JOIN PRESOS_ROUBO_FURTO RF ON (
        ...
        AND M.data_hora_fato > RF.data_hora_fato -- JUNÇÃO COM A TABELA QUE CALCULA OS PRESOS POR ROUBO E FURTO
    )
    LEFT JOIN PRESOS_VDOM VD ON (
        ...
        AND M.data_hora_fato > VD.data_hora_fato -- JUNÇÃO COM A TABELA QUE CALCULA OS PRESOS POR VIOLÊNCIA DOMÉSTICA
    )
