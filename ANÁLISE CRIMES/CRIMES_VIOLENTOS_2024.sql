...
( 
    SELECT 
        ...
        ENV.digitador_id_orgao,                   -- Seleciona o ID do órgão responsável pelo registro
        ...
        ENV.ind_militar_policial_servico          -- Seleciona indicador se militar estava em serviço
    FROM 
...
SELECT 
    ...
    ENV.numero_envolvido,                         -- Número do envolvido
    ...
    ENV.envolvimento_descricao,                   -- Descrição do envolvimento na ocorrência
...
FROM db_bisp_reds_reporting.tb_ocorrencia AS OCO -- From principal da tabela de ocorrências
INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia AS ENV ON OCO.numero_ocorrencia = ENV.numero_ocorrencia -- Join com a tabela de envolvidos
LEFT JOIN db_bisp_reds_master.tb_local_unidade_area_pmmg LO ON OCO.id_local = LO.id_local 
LEFT JOIN LETALIDADE LET 
    ON OCO.numero_ocorrencia = LET.numero_ocorrencia -- Left join com a CTE de letalidade
WHERE 1=1  
