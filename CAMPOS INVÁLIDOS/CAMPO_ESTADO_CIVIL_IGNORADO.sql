...
SELECT
    ...
    SUM(CASE 
        WHEN ENV.estado_civil_codigo IS NULL OR ENV.estado_civil_codigo ='9800' -- Verifica estado civil ignorado (ESTADO CIVIL - IGNORADO)
        THEN 1 ELSE 0 
        END) AS Qtd_Null_9800_9700 -- Soma os registros com estado civil nulo e ignorado
FROM db_bisp_reds_reporting.tb_ocorrencia OCO
JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV ON ENV.numero_ocorrencia = OCO.numero_ocorrencia -- junção das tabelas por número da ocorrência
WHERE 1 = 1
