/* ----------------------------------------------------------------------------------------------------------------------------------------------------
 *  ================================================================================================================================================
 *  =================================================== 2.2 VITIMIZAÇÃO DE MULHERES NO ÂMBITO DOMÉSTICO ============================================
 *  ================================================================================================================================================
 * 
 * O Indicador tem por finalidade avaliar a taxa de crimes com emprego de violência física ou sexual 
 * (Feminicídio, Homicídio, Tortura, Lesão Corporal, Estupro e Sequestro e Cárcere Privado) contra
 * mulheres no âmbito doméstico e familiar em relação à população feminina no estado de MinasGerais. 
 * São contabilizadas as vítimas mulheres em qualquer das naturezas retromencionadas,
 * que estejam com o marcador U33004 - Atendimento de Denúncia de Infrações Contra a Mulher (Violência Doméstica). 
 *-----------------------------------------------------------------------------------------------------------------------------------------------------*/
SELECT
   OCO.numero_ocorrencia,                                  -- Seleciona o identificador único da ocorrência
...
CASE
  WHEN OCO.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
   ...
   OCO.natureza_secundaria1_codigo,                        -- Seleciona o código da primeira natureza secundária
   OCO.natureza_secundaria2_codigo,                        -- Seleciona o código da segunda natureza secundária
   OCO.natureza_secundaria3_codigo,                        -- Seleciona o código da terceira natureza secundária
   OCO.natureza_consumado,                                 -- Indica se a natureza da ocorrência foi consumada ou tentada
   ENV.nome_completo_envolvido,                            -- Seleciona o nome completo do envolvido na ocorrência
   OCO.codigo_municipio,                                   -- Seleciona o código do município onde ocorreu o fato
   OCO.nome_municipio,                                     -- Seleciona o nome do município onde ocorreu o fato
   OCO.data_hora_fato,                                     -- Seleciona a data e hora em que o fato ocorreu
   YEAR(OCO.data_hora_fato) AS ano_fato                    -- Extrai o ano da data do fato e o rotula como “ano_fato”
FROM 
   db_bisp_reds_reporting.tb_ocorrencia AS OCO             -- Fonte principal: tabela de ocorrências, apelidada de OCO
LEFT JOIN 
   db_bisp_reds_reporting.tb_envolvido_ocorrencia AS ENV  -- Junção externa esquerda com tabela de envolvidos
   ON OCO.numero_ocorrencia = ENV.numero_ocorrencia        --   condição de junção: corresponde ao mesmo número de ocorrência
WHERE 1 = 1
    ...
    AND OCO.digitador_sigla_orgao IN ('PM', 'PC')          -- Inclui somente registros digitados pela Polícia Militar (PM) ou Civil (PC)
    AND OCO.nome_tipo_relatorio IN ('POLICIAL', 'REFAP')   -- Seleciona tipos de relatório “POLICIAL” ou “REFAP”
    AND (
        (
        ...
        ) ---  Homicídio, Tortura, Lesão Corporal, Sequestro e Cárcere Privado, Feminicídio e Estupro
        AND
        (
        ...
        ) --  Atendimento de Denúncia de Infrações Contra a Mulher (Violência Doméstica).
    )
    AND (
        ENV.codigo_sexo = 'F'   OR identidade_genero_codigo IN ('0400','0200','0700','0100','0600')  --Inclui somente envolvidas do sexo feminino Ou com outras identidades de gênero especificadas
    	)
    AND ENV.envolvimento_codigo IN ('1300', '1399', '1301', '1302', '1303', '1304', '1305')-- Filtra apenas vítimas 
    AND ENV.id_relacao_vitima_autor IN (3,4,9,15,16,18,22)-- Restringe aos relacionamentos autor-vitima de âmbito doméstico;
