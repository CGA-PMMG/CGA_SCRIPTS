 ...
 * para o ano de 2024, com o foco em prisões efetuadas pela Polícia Militar de Minas Gerais. O código está estruturado 
 * para agrupar e contar o total de envolvidos por unidade operacional, baseando-se em diferentes 
 * faixas etárias (menores e maiores de idade).
 ----------------------------------------------------------------------------------------------------------------------*/
...
CASE WHEN OCO.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		ELSE 'OUTROS' 
	END AS UEOP_2024,	
-- CONTAGEM DO NÚMERO DE ENVOLVIDOS EM CADA UNIDADE
COUNT(ENV.numero_envolvido),
-- DETERMINAÇÃO DA FAIXA ETÁRIA BASEADA NA DIFERENÇA ENTRE O ANO DO FATO E O ANO DE NASCIMENTO
    CASE 
        WHEN (YEAR(ENV.data_hora_fato) - YEAR(ENV.data_nascimento)) < 18 THEN 'MENOR IDADE'
        ELSE 'MAIOR IDADE'
    END AS FAIXA_ETARIA
...
AND ENV.tipo_prisao_apreensao_codigo IN ('9900', '0400', '0100', '0300', '0600', '0200')
--AND OCO.unidade_area_militar_nome LIKE '%1 CIA PM IND/3 RPM%' -- FILTRE SUA CIA/BPM/RPM
-- AGRUPAMENTO DOS RESULTADOS POR UNIDADE OPERACIONAL E FAIXA ETÁRIA PARA CONTAGEM CORRETA
GROUP BY 1, 2, 4
-- ORDENAÇÃO DOS RESULTADOS POR UNIDADE E FAIXA ETÁRIA PARA APRESENTAÇÃO CLARA
ORDER BY 1, 2, 4
