/*------------------------------------------------------------------------------------------------------------------------------------------------------
 * Este código SQL é projetado para identificar autores presos. A consulta é realizada nas tabelas de ocorrências e envolvidos, filtrando por 
 * tipos específicos de naturezas de crime e garantindo que os envolvidos tenham informações pessoais completas.
 -----------------------------------------------------------------------------------------------------------------------------------------------------*/
WITH
PRESOS_MV AS  -- Define uma CTE (Common Table Expression) chamada PRESOS_MV
(
	SELECT 
...
FROM 
	...
	INNER JOIN PRESOS_MV PMV ON OCO.numero_ocorrencia = PMV.numero_ocorrencia -- Associação das ocorrências com os envolvidos
...
WHERE 1=1
	AND OCO.digitador_id_orgao IN (0,1) -- Registro feito por órgãos específicos(PM , PC)
