 ...
 * - Filtragem e seleção de ocorrências com base em palavras-chave específicas no histórico descritivo;
 * - Extração e organização dos dados relevantes para subsidiar análises;
 *
 * Este script destina-se a apoiar análises operacionais e estratégicas, contribuindo para o aprimoramento da gestão 
 * e da tomada de decisão.
 *---------------------------------------------------------------------------------------------------------------------*/
SELECT 
    OCO.numero_ocorrencia,  -- Número ocorrência
    ...
    OCO.natureza_ind_consumado, -- Indica se a natureza foi consumada
    OCO.nome_municipio,  -- Nome do municipio 
...
WHERE 1=1 -- Filtro genérico para adicionar condições subsequentes
   ...
   AND OCO.historico_ocorrencia LIKE '%xxxxxxxxxx%'  -- Filtro por texto específico no campo 'histórico da ocorrência'    
