/*----------------------------------------------------------------------------------------------------------------------------
* Este script foi desenvolvido para realizar uma extração detalhada de ocorrências policiais registradas na base de dados REDS,
* com foco específico nas ocorrências por UEOP. O script coleta informações abrangentes sobre cada ocorrência, incluindo dados
* temporais (data, hora, dia da semana), dados geográficos (município, bairro, coordenadas), natureza do fato e detalhes do local.
* Além disso, realiza um mapeamento complexo das Regiões da Polícia Militar (RPM) e Unidades Operacionais (UEOP) com base nos 
* códigos dos municípios, permitindo uma análise organizacional da distribuição das ocorrências. Os filtros aplicados garantem
* que apenas ocorrências fechadas, registradas pela PM em Minas Gerais, dos tipos POLICIAL, RAT e BOS sejam consideradas,
* dentro do período especificado pelos parâmetros de ano e mês.
*---------------------------------------------------------------------------------------------------------------------------*/
SELECT OCO.numero_ocorrencia,                               -- Identificador único da ocorrência policial
OCO.data_hora_fato,                                        -- Data e horário em que o fato ocorreu 
YEAR(OCO.data_hora_fato) ANO,                             -- Extração do ano da data do fato
MONTH(OCO.data_hora_fato) MES,                            -- Extração do mês da data do fato
DAY(OCO.data_hora_fato) DIA,                              -- Extração do dia da data do fato
DAYOFWEEK(OCO.data_hora_fato) DIA_SEMANA_OCORRENCIA,      -- Extração do dia da semana (1-7) da data do fato
OCO.natureza_codigo,                                       -- Código identificador da natureza da ocorrência 
OCO.natureza_descricao,                                    -- Descrição textual da natureza da ocorrência
OCO.natureza_ind_consumado,                                -- Indicador se a natureza foi consumada (S/N)
OCO.natureza_consumado,                                    -- Descrição textual do status de consumação
OCO.numero_latitude,                                       -- Coordenada de latitude do local do fato
OCO.numero_longitude,                                      -- Coordenada de longitude do local do fato
OCO.codigo_municipio,                                      -- Código IBGE do município do fato
OCO.nome_municipio,                                        -- Nome do município do fato
OCO.codigo_bairro,                                         -- Código identificador do bairro do fato
OCO.nome_bairro,                                           -- Nome do bairro onde ocorreu o fato
OCO.tipo_local_codigo,                                     -- Código do tipo de local do fato
OCO.tipo_local_descricao,                                  -- Descrição do tipo de local do fato
OCO.tipo_local_descricao_longa,                           -- Descrição detalhada do tipo de local
OCO.tipo_logradouro_codigo,                               -- Código do tipo de logradouro (rua, avenida etc)
OCO.tipo_logradouro_descricao,                            -- Descrição do tipo de logradouro
OCO.logradouro_codigo,                                     -- Código identificador do logradouro
OCO.logradouro_nome,                                       -- Nome do logradouro do fato
OCO.descricao_endereco,                                    -- Descrição textual completa do endereço
OCO.numero_endereco,                                       -- Número do endereço do fato
OCO.logradouro2_codigo,                                    -- Código do logradouro secundário 
OCO.logradouro2_nome,                                      -- Nome do logradouro secundário
OCO.local_mediato_codigo,                                  -- Código do local mediato do fato
OCO.local_mediato_descricao,                              -- Descrição do local mediato
OCO.local_mediato_descricao_longa,                        -- Descrição detalhada do local mediato
OCO.local_imediato_codigo,                                -- Código do local imediato do fato
OCO.local_imediato_descricao,                             -- Descrição do local imediato
OCO.local_imediato_longa,                                 -- Descrição detalhada do local imediato
...
OCO.digitador_sigla_orgao,                                -- Sigla do órgão do responsável pelo registro
OCO.digitador_codigo_usuario,                             -- Código do usuário que registrou a ocorrência
OCO.ocorrencia_uf,                                        -- Unidade federativa da ocorrência
OCO.nome_tipo_relatorio, 					 			  -- Nome do tipo de relatório (REDS/RAT/BOS)
OCO.ind_estado                                            -- Indicador do estado da ocorrência (A-Aberta/F-Fechada)
FROM db_bisp_reds_reporting.tb_ocorrencia OCO             -- Tabela principal de ocorrências
WHERE 1=1                                                 -- Início das condições de filtro
AND OCO.ocorrencia_uf = 'MG'                             -- Filtro para ocorrências de Minas Gerais
AND OCO.digitador_sigla_orgao = 'PM'                     -- Filtro para registros feitos pela PM
AND OCO.nome_tipo_relatorio IN ('POLICIAL','RAT','BOS')  -- Filtro para tipos específicos de relatório(REDS/RAT/BOS)
AND YEAR(OCO.data_hora_fato) = :ANO                      -- Filtro pelo ano parametrizado
AND MONTH(OCO.data_hora_fato) >= :MESINICIAL             -- Filtro pelo mês inicial parametrizado
AND MONTH(OCO.data_hora_fato) <= :MESFINAL               -- Filtro pelo mês final parametrizado
--AND OCO.unidade_area_militar_nome LIKE '%X BPM/%'       -- Filtro pelo nome da unidade da área militar
--AND OCO.unidade_responsavel_registro_nome LIKE '%X BPM/%'       -- Filtro pelo nome da unidade responsável pelo registro 
--AND OCO.codigo_municipio IN('0','0') -- Filtro pelo nome do município do fato
AND OCO.ind_estado IN ('F')                              -- Filtro para ocorrências fechadas
ORDER BY OCO.data_hora_fato, OCO.numero_ocorrencia