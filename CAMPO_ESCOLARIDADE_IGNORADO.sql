...
SELECT
    ...
    SPLIT_PART(OCO.unidade_responsavel_registro_nome, '/', -1) AS N1, -- EXTRAI O ÚLTIMO SEGMENTO DA UNIDADE (MAIS ESPECÍFICO)
    SPLIT_PART(OCO.unidade_responsavel_registro_nome, '/', -2) AS N2, -- EXTRAI O PENÚLTIMO SEGMENTO DA UNIDADE
    SPLIT_PART(OCO.unidade_responsavel_registro_nome, '/', -3) AS N3, -- EXTRAI O ANTEPENÚLTIMO SEGMENTO DA UNIDADE
...
WHERE YEAR(OCO.data_hora_fato) = 2024  -- FILTRA OS DADOS PARA OCORRÊNCIAS DO ANO 2024
  ...
  AND ENV.condicao_fisica_codigo NOT IN ('0300', '0100') -- EXCLUI CONDIÇÕES FÍSICAS GRAVES OU FATAIS
   -- AND OCO.unidade_responsavel_registro_nome LIKE '%/1 BPM%' -- FILTRA OCORRÊNCIAS RELACIONADAS A UNIDADE DE REGISTRO.
