 ...
 * registrados em Minas Gerais, excluindo casos de letalidade policial. A consulta integra informações das vítimas e ocorrências, 
...
SELECT 
    ...
    UPPER(ENV.nome_completo_envolvido) nome_completo_envolvido,                  -- Nome completo do envolvido
...
INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia AS ENV ON OCO.numero_ocorrencia = ENV.numero_ocorrencia -- Join com a tabela de envolvidos
 LEFT JOIN LETALIDADE LET  ON OCO.numero_ocorrencia = LET.numero_ocorrencia -- Left join com a CTE de letalidade
