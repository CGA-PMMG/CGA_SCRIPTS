/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*
* Este script SQL tem por finalidade extrair e padronizar um conjunto de dados analíticos sobre coletes à prova de bala apreendidos em ocorrências registradas em Minas Gerais, no período 
* de 1º de setembro de 2023 a 18 de setembro de 2025. A análise busca subsidiar o planejamento operacional, a gestão de riscos e a produção de conhecimento tático-estratégico.
*
 -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
SELECT OCO.numero_ocorrencia,  -- Número da ocorrência
ENV.numero_envolvido, -- Número do envolvido
ENV.ind_militar_policial, -- Indica se o envolvido é policial militar em serviço
...
UPPER(ENV.nome_completo_envolvido) nome_completo_envolvido,        -- Nome completo da pessoa envolvida na ocorrência
UPPER(ENV.nome_mae) nome_mae,      -- Nome da mãe do envolvido
CONCAT(
...
ENV.numero_documento_id,  -- Número de identidade do envolvido
ENV.numero_cpf_cnpj, -- Número de CPF/CNPJ do envolvido
ENV.codigo_municipio codigo_municipio_envolvido,  -- Código do munícipio do envolvido
ENV.nome_municipio nome_municipio_envolvido, -- Nome do munícipio do envolvido
MAT.numero_material , -- Número do materialapreendido
MAT.quantidade_material, -- Quantidade do material apreendido
MAT.unidade_medida_descricao, -- Descrição da unidade de medida do material apreendido
MAT.tipo_objeto_codigo,  -- Código do tipo do material apreendido
MAT.tipo_objeto_descricao, -- Descrição do tipo do material apreendido
MAT.situacao_codigo, -- Código da situação do material : apreendido
MAT.situacao_descricao, -- Descrição da situação do material : apreendido
ENV.natureza_ocorrencia_codigo natureza__envolvido_codigo,  -- Código da natureza do envolvido na ocorrência
ENV.natureza_ocorrencia_descricao natureza_envolvido_descricao,  -- Descrição da natureza do envolvido na ocorrência
OCO.natureza_codigo natureza_ocorrencia_codigo,   -- Código da natureza da ocorrência                                     
...
INNER JOIN db_bisp_reds_reporting.tb_material_apreendido_ocorrencia MAT ON  ENV.numero_ocorrencia = MAT.numero_ocorrencia AND ENV.numero_envolvido = MAT.numero_envolvido  
...
WHERE 1 = 1
	AND ENV.id_envolvimento = 35 -- Filtra envolvimento igual a 35 (Autor)
	AND MAT.tipo_objeto_codigo IN ('2007', '2056') -- Filtra o tipo de objeto Colete : COLETES CONTRA ARMAS DE FOGO DE USO PERMITIDO ou COLETES CONTRA ARMAS DE FOGO DE USO RESTRITO
	AND MAT.situacao_codigo = '0100'
	AND OCO.digitador_sigla_orgao IN ('PM', 'PC')  -- Filtra ocorrências registradas pela Polícia Militar ou Polícia Civil
    AND OCO.ocorrencia_uf = 'MG'  -- Filtra ocorrências registradas no Estado de Minas Gerais
    AND OCO.ind_estado = 'F' -- Filtra ocorrências com estago = Fechado
   AND OCO.data_hora_fato BETWEEN '2023-09-01 00:00:00.000'  AND '2025-09-18 23:59:59.000'-- Filtra intervalos específicado
