 ...
 * das unidades. Versão 10/04/2024
 ----------------------------------------------------------------------------------------------------------------------------------*/
------ ESTA SELECÇÃO RETORNA DADOS DE OCORRÊNCIAS DO GRUPO B01 E C01 QUE CONTABILIZAM PARA O IPRP(DENOMINADOR) (GDO) -------
SELECT 
		...
		LEFT(oco.natureza_codigo,3) as Natureza, -- SELECIONA OS TRÊS PRIMEIROS DIGITOS DA ESQUERDA
		CAST(oco.codigo_municipio AS INTEGER) AS codigo_municipio , -- SELECIONA E CONVERTE O CÓDIGO DE MUNICIPIO PARA INTEIRO
...
CASE WHEN oco.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
	...
	oco.numero_ocorrencia,
		COUNT(DISTINCT  CASE WHEN (
			LEFT(oco.natureza_codigo,3) IN ('B01','C01')  -- FILTRA NATUREZAS DE OCORRÊNCIA QUE INICIAM COM 'B01' E 'C01'
			AND oco.ind_estado ='F' -- FILTRA ESTADO DA OCORRÊNCIA FECHADO
			AND ocorrencia_uf ='MG'  -- FILTRA OCORRÊNCIAS CUJA UF É MG
			AND oco.digitador_sigla_orgao  IN ('PM','PC')-- FILTRA OCORRÊNCIAS REGISTRADAS PELA PM OU PC
			)  
			THEN oco.numero_ocorrencia  END) AS 'REG_IPRP' -- CONTADOR QUE REGISTRA A QUANTIDADE DE OCORRÊNCIAS DENTRO DOS CRITÉRIOS ESPECÍFICADOS 
FROM db_bisp_reds_reporting.tb_ocorrencia AS oco
WHERE	1=1  -- CONDIÇÃO SEMPRE VERDADEIRA PARA FACILITAR A ADIÇÃO DE OUTRAS CONDIÇÕES
	AND oco.digitador_sigla_orgao IN  ( 'PM','PC') -- FILTRA OCORRÊNCIAS REGISTRADAS PELA PM OU PC
	AND 	oco.ocorrencia_uf  IN  ('MG') -- FILTRA OCORRÊNCIAS CUJA UF É MG
	AND 	LEFT(oco.natureza_codigo,3) IN ('B01','C01')  -- FILTRA NATUREZAS DE OCORRÊNCIA QUE INICIAM COM 'B01' E 'C01' 
	AND YEAR(oco.data_hora_fato) = :ANO -- PROMPT PARA FILTRAR O ANO DA DATA/HORA FATO
	AND MONTH (oco.data_hora_fato) >= :MESINICIAL -- PROMPT PARA FILTRAR O MÊS INICIAL DA DATA/HORA FATO
	AND MONTH (oco.data_hora_fato) <= :MESFINAL -- PROMPT PARA FILTRAR O MÊS FINAL DA DATA/HORA FATO
	AND 	oco.ind_estado IN ('F') -- FILTRA O ESTADO DA OCORRÊNCIA COMO FECHADO
--	AND oco.data_hora_inclusao < '2024-02-06' 
