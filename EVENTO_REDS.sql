SELECT EV.id_evento, -- IDENTIFICADOR DO EVENTO 
EV.descricao_evento, -- DESCRICAO DO EVENTO
OCO.numero_ocorrencia, -- NUMERO DO  REDS
OCO.data_hora_fato, -- DATO HORA DO FATO
HOUR(OCO.data_hora_fato) HORA_FATO, -- HORA DO FATO
CA.chamada_numero, -- NUMERO DA CHAMADA CAD
...
WHERE 
    CA.orgao_sigla = 'PM' -- FILTRA PELA SIGLA DA POLÍCIA MILITAR
    ...
    AND EV.id_evento IS NOT NULL -- FILTRA PELO ID DO EVENTO (ENQUANTO NULL, RETORNA APENAS REDS QUE TENHAM EVENTO. PODE TAMBÉM RETORNAR EVENTO ESPECIFICO UTILIZANDO O CODIGO IDENTIFICADOR)
    AND EV.descricao_evento = 'VIOLÊNCIA DOMÉSTICA' -- FILTRA PELA DESCRIÇÃO DO EVENTO
