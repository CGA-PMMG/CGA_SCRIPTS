/*----------------------------------------------------------------------------------------------------------------------
 O código SQL a seguir está projetado para retornar informações sobre o envolvimento de menores de idade em 
 * ocorrências registradas em Minas Gerais no ano de 2024. O objetivo é identificar todos os tipos de envolvimento 
...
SELECT  
    ...
    CASE 
        WHEN ENV.valor_idade_aparente < 18 THEN 'MENOR IDADE' -- Classifica como menor de idade se a idade aparente for menor que 18.
        ELSE 'MAIOR IDADE' -- Classifica como maior de idade caso contrário.
    END AS FAIXA_ETARIA, -- Determina a faixa etária do envolvido com base na diferença entre o ano do fato e o ano de nascimento.
    ENV.valor_idade_aparente valor_idade_aparente_reds, -- Valor da idade aparente do envolvido, conforme registro.
    ENV.envolvimento_descricao, -- Descrição do tipo de envolvimento do indivíduo na ocorrência.
    ENV.natureza_ocorrencia_codigo AS natureza_envolvido, -- Código da natureza relacionada ao envolvido.
    ENV.natureza_ocorrencia_descricao as natureza_envolvido_descricao, -- Código da natureza da ocorrência.
    ENV.condicao_fisica_descricao, -- Descrição da condição física do envolvido no momento da ocorrência.
...
CASE WHEN OCO.codigo_municipio in (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
        ...
        WHEN OCO.codigo_municipio =317020 AND (LO.unidade_area_militar_nome like '32 BPM%' or LO.unidade_area_militar_nome like '%/32 BPM%') THEN '32 BPM'
        WHEN OCO.codigo_municipio =317020 AND (LO.unidade_area_militar_nome like '17 BPM%' or LO.unidade_area_militar_nome like '%/17 BPM%') THEN '17 BPM'
        WHEN OCO.codigo_municipio =317010 AND (LO.unidade_area_militar_nome like '4 BPM%' or LO.unidade_area_militar_nome like '%/4 BPM%') THEN '04 BPM'
        WHEN OCO.codigo_municipio =317010 AND (LO.unidade_area_militar_nome like '67 BPM%' or LO.unidade_area_militar_nome like '%/67 BPM%') THEN '67 BPM'
        WHEN OCO.codigo_municipio =314330 AND (LO.unidade_area_militar_nome like '50 BPM%' or LO.unidade_area_militar_nome like '%/50 BPM%') THEN '50 BPM'
        WHEN OCO.codigo_municipio =314330 AND (LO.unidade_area_militar_nome like '10 BPM%' or LO.unidade_area_militar_nome like '%/10 BPM%') THEN '10 BPM'
        WHEN OCO.codigo_municipio =313670 AND (LO.unidade_area_militar_nome like '27 BPM%' or LO.unidade_area_militar_nome like '%/27 BPM%') THEN '27 BPM'
        WHEN OCO.codigo_municipio =313670 AND (LO.unidade_area_militar_nome like '2 BPM%' or LO.unidade_area_militar_nome like '%/2 BPM%') THEN '02 BPM'
        ...
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '1 BPM%' or LO.unidade_area_militar_nome like '%/1 BPM%') THEN '01 BPM'
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '5 BPM%' or LO.unidade_area_militar_nome like '%/5 BPM%')  THEN '05 BPM'
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '13 BPM%' or LO.unidade_area_militar_nome like '%/13 BPM%')  THEN '13 BPM'
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '16 BPM%' or LO.unidade_area_militar_nome like '%/16 BPM%')  THEN '16 BPM'
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '22 BPM%' or LO.unidade_area_militar_nome like '%/22 BPM%' )  THEN '22 BPM'
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '41 BPM%' or LO.unidade_area_militar_nome like '%/41 BPM%') THEN '41 BPM'
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '49 BPM%' or LO.unidade_area_militar_nome like '%/49 BPM%') THEN '49 BPM'
        WHEN OCO.codigo_municipio =310620 AND (LO.unidade_area_militar_nome like '34 BPM%' or LO.unidade_area_militar_nome like '%/34 BPM%') THEN '34 BPM'
        WHEN OCO.codigo_municipio =316620 AND (LO.unidade_area_militar_nome like '31 BPM%' or LO.unidade_area_militar_nome like '%/31 BPM%') THEN '31 BPM'
...
WHERE 1=1 -- Condição inicial para facilitar adição de filtros subsequentes.
	AND OCO.data_hora_fato between '2024-01-01 00:00:00.000' and '2024-11-25 23:59:59.000' -- Filtra ocorrências dentro do intervalo de datas especificado.
	AND ENV.id_envolvimento IN (872,25,1097,27,32,26,28,36,35) -- Filtra por tipos específicos de envolvimento. Todos vitima, autor e co-autor
	AND ENV.valor_idade_aparente < 18 -- Filtra apenas registros de menores de idade.
	--AND OCO.natureza_codigo ('x') -- Filtre pela natureza desejada	
	AND OCO.ocorrencia_uf ='MG' -- Filtra ocorrências na Unidade Federativa de Minas Gerais.
    AND OCO.digitador_sigla_orgao IN ('PM','PC') -- Filtra registros feitos pela Polícia Militar (PM) ou Polícia Civil (PC).
    AND OCO.nome_tipo_relatorio IN ('POLICIAL','REFAP') -- Filtra tipos de relatórios específicos.
    AND OCO.ind_estado = 'F' -- Filtra ocorrências fechadas.
...
ORDER BY OCO.data_hora_fato, OCO.numero_ocorrencia; -- Ordena os resultados por RPM, UEOP e número da ocorrência.
