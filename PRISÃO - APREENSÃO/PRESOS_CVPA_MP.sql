/*------------------------------------------------------------------------------------------------------------------------------------------------------
 * Este código SQL é projetado para identificar autores presos pelos crimes do Indicador de Resultado **** CVPA(Crime Violento Contra o Patrimônio) *** 
 * da GDO 2025 com mandado de prisão em aberto.
 * A consulta é realizada nas tabelas de ocorrências e envolvidos, filtrando por naturezas CVPA(Crime Violento Contra o Patrimônio) e garantindo que
 *  os envolvidos tenham informações pessoais completas.
  
 -----------------------------------------------------------------------------------------------------------------------------------------------------*/

WITH
PRESOS_MP_CVPA AS  -- Define uma CTE (Common Table Expression) chamada PRESOS_MP_CVPA - Autores, Co-Autores e Suspeitos de crimes de Morte Violenta
(
	SELECT 
		...
		ENV.ind_militar_policial, -- Indicador se o envolvido é militar ou policial
		ENV.digitador_id_orgao,  -- Identificação do órgão responsável pelo registro
		ENV.tipo_prisao_apreensao_descricao, -- Descrição do tipo de prisão ou apreensão
		ENV.condicao_fisica_descricao, -- Condição física do envolvido
		ENV.natureza_ocorrencia_codigo, -- Código da natureza da ocorrência associada ao envolvido
		ENV.numero_individuo_sip, -- Número SIP do envolvido 
		ENV.ind_mandado_prisao, -- Indica se o envolvido tem mandado de prisão em aberto
		ENV.ind_mandado_prisao_atual -- Indica de o envolvido tem mandado de prisão atual em aberto 
    FROM db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV -- Origem dos dados: tabela de envolvidos nas ocorrências
	WHERE 1=1
	AND (
            ENV.ind_mandado_prisao_atual = 'S' -- FILTRA INDICADOR DE MANDATO DE PRISÃO ATUAL COMO 'SIM'
            OR ENV.ind_mandado_prisao = 'S' -- FILTRA INDICADOR DE MANDATO DE PRISÃO COMO 'SIM'
            )
		AND ENV.id_envolvimento IN(35,36,44) -- Filtra somente envolvidos classificados como autor, co-autor, suspeito
		AND ENV.natureza_ocorrencia_codigo IN ('C01157','C01158','C01159') -- Filtra pela natureza específica da ocorrência - CVPA
        ...
        AND ENV.numero_ocorrencia IN (
						         SELECT 
						         		vit.numero_ocorrencia -- Número da ocorrência
						         FROM
						         		db_bisp_reds_reporting.tb_envolvido_ocorrencia vit -- Tabela de envolvidos nas ocorrências
						         WHERE 1=1 
						        	AND vit.id_envolvimento IN (25,32,1097,26,27,28,872) -- Filtra por tipos de vítima
									AND vit.natureza_ocorrencia_codigo IN ('C01157','C01158','C01159') -- A vítima deve ter a mesma natureza da ocorrência que o autor - CVPA
									AND vit.digitador_id_orgao IN(0) -- Registro feito por órgãos específicos -PM
...
SELECT 
	...
	PMV.numero_envolvido, -- Número do envolvido na ocorrência
	PMV.nome_completo_envolvido,  -- Nome completo do envolvido
	PMV.nome_mae, -- Nome da mãe do envolvido
	PMV.data_nascimento, -- Data de nascimento do envolvido
	PMV.envolvimento_descricao, -- Tipo de envolvimento do indivíduo
	PMV.tipo_prisao_apreensao_descricao, -- Tipo de prisão ou apreensão
	PMV.numero_individuo_sip, -- Número SIP do envolvido 
	PMV.ind_mandado_prisao, -- Indica se o envolvido tem mandado de prisão em aberto
	PMV.ind_mandado_prisao_atual, -- Indica de o envolvido tem mandado de prisão atual em aberto 
...
CASE WHEN OCO.codigo_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		ELSE 'OUTROS' 
	END AS UEOP_2024,	
	CAST(OCO.codigo_municipio AS INTEGER) AS codigo_municipio, -- Código do município
	OCO.nome_municipio, -- Nome do município
	OCO.nome_bairro, -- Nome do bairro
	OCO.descricao_endereco, -- Endereço onde ocorreu o fato
	OCO.numero_endereco, -- Número do endereço
	OCO.unidade_area_militar_codigo, -- Código da unidade militar responsável
	OCO.unidade_area_militar_nome, -- Nome da unidade militar responsável
	OCO.unidade_responsavel_registro_codigo, -- Código da unidade responsável pelo registro
	OCO.unidade_responsavel_registro_nome, -- Nome da unidade responsável pelo registro
	OCO.digitador_sigla_orgao -- Sigla do órgão responsável pelo registro
FROM 
	db_bisp_reds_reporting.tb_ocorrencia AS OCO -- Origem dos dados: tabela de ocorrências
	INNER JOIN PRESOS_MP_CVPA PMV ON OCO.numero_ocorrencia = PMV.numero_ocorrencia -- Associação das ocorrências com os envolvidos
WHERE 1=1
	AND OCO.ind_estado ='F' -- Somente ocorrências fechadas
	AND OCO.ocorrencia_uf ='MG' -- Considera apenas ocorrências no estado de Minas Gerais
	AND OCO.nome_tipo_relatorio IN ('POLICIAL') -- Filtra tipos específicos de relatórios
	AND OCO.digitador_id_orgao IN (0) -- Registro feito por órgãos específicos(PM)
	AND OCO.data_hora_fato BETWEEN '2025-01-01 00:00:00.000' AND '2025-02-11 23:59:59.000' -- Filtra data/hora fato dentro do intervalo especificado 
