/*------------------------------------------------------------------------------------------------------------------------------------------------------
 * Este código SQL é projetado para identificar autores presos pelos crimes do Indicador de Resultado **** CVPE(Crime Violento Contra Pessoa) *** 
a GDO 2025 com mandado
 * de prisão em aberto.
 * A consulta é realizada nas tabelas de ocorrências e envolvidos, filtrando por naturezas CVPE(Crime Violento Contra Pessoa) e garantindo que os envolvidos
 *  tenham informações pessoais completas.
  
 -----------------------------------------------------------------------------------------------------------------------------------------------------*/

WITH
PRESOS_MP_CVPE AS  -- Define uma CTE (Common Table Expression) chamada PRESOS_MP_CVPE - Autores, Co-Autores e Suspeitos de crimes de Morte Violenta
(
	SELECT 
	...
	AND (
		...
		AND ENV.natureza_ocorrencia_codigo IN ('B01121','B01148','B02001','B01504') -- Filtra pela natureza específica da ocorrência - CVPE
        ...
        AND ENV.numero_ocorrencia IN (
						         ...
						         WHERE 1=1 
									...
									AND vit.natureza_ocorrencia_codigo IN ('B01121','B01148','B02001','B01504') -- A vítima deve ter a mesma natureza da ocorrência que o autor - CVPE
...
FROM 
	...
	INNER JOIN PRESOS_MP_CVPE PMV ON OCO.numero_ocorrencia = PMV.numero_ocorrencia -- Associação das ocorrências com os envolvidos
WHERE 1=1
