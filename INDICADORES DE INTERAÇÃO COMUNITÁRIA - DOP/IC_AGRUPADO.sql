...
BASE AS (  -- Define a CTE para extrair informações de ocorrências com número do BO no histórico
  ...
  WHERE 
    ...
    AND data_hora_fato >= '2025-01-01 00:00:00'  -- Filtra ocorrências a partir de 1º de janeiro de 2024
...
END AS VALIDO_FURTO_RESIDCOM,
   CASE
   	WHEN natureza_codigo IN ('B01121','B01148','B02001','C01157','C01158','D01217','B01504') THEN 'VALIDO' ELSE 'INVALIDO'
   END AS VALIDO_CV   
  FROM db_bisp_reds_reporting.tb_ocorrencia OCO
  WHERE natureza_codigo IN ('C01155','B01121','B01148','B02001','C01157','C01158','D01217','B01504') -- Filtra apenas naturezas de Furto e CV  
...
COUNT(
    CASE 
      WHEN ENV.numero_cpf_cnpj IS NOT NULL 
        OR (ENV.tipo_documento_codigo = '0801' AND ENV.numero_documento_id IS NOT NULL) -- IDENTIDADE CIVIL
        OR (ENV.tipo_documento_codigo = '0809' AND ENV.numero_documento_id IS NOT NULL) -- CART. PROFISSIONAL
        OR (ENV.tipo_documento_codigo = '0802' AND ENV.numero_documento_id IS NOT NULL) -- CART. TRABALHO
        OR (ENV.tipo_documento_codigo = '0803' AND ENV.numero_documento_id IS NOT NULL) -- CNH
      THEN 1 
      ELSE NULL 
    END
  ) AS qtd_envolvidos_identificados -- Conta a quantidade de envolvidos validos	
  FROM db_bisp_reds_reporting.tb_ocorrencia OCO
...
OCO.id_local,		--  Identificador numérico do local 
L.codigo_unidade_area,                              -- Código da unidade militar da área
L.nome_unidade_area,                                -- Nome da unidade militar da área
...
SPLIT_PART(OCO.unidade_responsavel_registro_nome,'/',-1) RPM_REGISTRO,  -- Extrai parte após '/' como sigla RPM no registro
SPLIT_PART(OCO.unidade_responsavel_registro_nome,'/',-2) UEOP_REGISTRO,  -- Extrai parte antes de '/' como sigla UEOP no registro
CASE
...
CASE WHEN oco.codigo_municipio in (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
       ...
       WHEN oco.codigo_municipio =317020 AND (L.nome_unidade_area like '32 BPM%' or L.nome_unidade_area like '%/32 BPM%') THEN '32 BPM'
        ...
        WHEN oco.codigo_municipio =314330 AND (L.nome_unidade_area like '50 BPM%' or L.nome_unidade_area like '%/50 BPM%') THEN '50 BPM'
        WHEN oco.codigo_municipio =314330 AND (L.nome_unidade_area like '10 BPM%' or L.nome_unidade_area like '%/10 BPM%') THEN '10 BPM'
        WHEN oco.codigo_municipio =313670 AND (L.nome_unidade_area like '27 BPM%' or L.nome_unidade_area like '%/27 BPM%') THEN '27 BPM'
        WHEN oco.codigo_municipio =313670 AND (L.nome_unidade_area like '2 BPM%' or L.nome_unidade_area like '%/2 BPM%') THEN '02 BPM'
        WHEN oco.codigo_municipio =311860 AND (L.nome_unidade_area like '39 BPM%' or L.nome_unidade_area like '%/39 BPM%') THEN '39 BPM'
        WHEN oco.codigo_municipio =311860 AND (L.nome_unidade_area like '18 BPM%' or L.nome_unidade_area like '%/18 BPM%') THEN '18 BPM'
        WHEN oco.codigo_municipio =310670 AND (L.nome_unidade_area like '66 BPM%' or L.nome_unidade_area like '%/66 BPM%') THEN '66 BPM'
        WHEN oco.codigo_municipio =310670 AND (L.nome_unidade_area like '33 BPM%' or L.nome_unidade_area like '%/33 BPM%') THEN '33 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '1 BPM%' or L.nome_unidade_area like '%/1 BPM%') THEN '01 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '5 BPM%' or L.nome_unidade_area like '%/5 BPM%')  THEN '05 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '13 BPM%' or L.nome_unidade_area like '%/13 BPM%')  THEN '13 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '16 BPM%' or L.nome_unidade_area like '%/16 BPM%')  THEN '16 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '22 BPM%' or L.nome_unidade_area like '%/22 BPM%' )  THEN '22 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '41 BPM%' or L.nome_unidade_area like '%/41 BPM%') THEN '41 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '49 BPM%' or L.nome_unidade_area like '%/49 BPM%') THEN '49 BPM'
        WHEN oco.codigo_municipio =310620 AND (L.nome_unidade_area like '34 BPM%' or L.nome_unidade_area like '%/34 BPM%') THEN '34 BPM'
        WHEN oco.codigo_municipio =316620 AND (L.nome_unidade_area like '31 BPM%' or L.nome_unidade_area like '%/31 BPM%') THEN '31 BPM'
        WHEN oco.codigo_municipio =316620 AND (L.nome_unidade_area like '9 BPM%' or L.nome_unidade_area like '%/9 BPM%') THEN '09 BPM'
        ELSE 'OUTROS'
...
CONCAT (CAST(OCO.numero_latitude as STRING),
	CAST(OCO.numero_longitude as STRING)) AS CHAVE_LATLONG,
...
CASE 
	WHEN OCO.natureza_codigo = 'A21000' AND OCO.data_hora_fato BETWEEN '2025-01-01 00:00:00.000' AND '2025-07-31 23:59:59.000' AND qtd_envolvidos_identificados >= 1 THEN 1
...
CASE 
	WHEN OCO.natureza_codigo IN ('A19000', 'A19001','A19004','A19099') AND qtd_envolvidos_identificados >= 3 THEN 1 ELSE 0
END AS RC_TOTAL, -- Atribui 1 se condição atendida, caso contrário 0
CASE 
	WHEN OCO.natureza_codigo IN ('A19006', 'A19007','A19008','A19009', 'A19010', 'A19011') AND qtd_envolvidos_identificados >= 3 THEN 1 ELSE 0
END AS MRPP_TOTAL,  -- Atribui 1 se condição atendida, caso contrário 0
CASE 
	WHEN OCO.natureza_codigo = 'A20000' AND OCO.data_hora_fato BETWEEN '2025-01-01 00:00:00.000' AND '2025-07-31 23:59:59.000' AND qtd_envolvidos_identificados >= 1  AND VALIDO_FURTO_RESIDCOM = 'VALIDO' THEN 1
	WHEN OCO.natureza_codigo = 'A20028' AND qtd_envolvidos_identificados >= 1  AND VALIDO_FURTO_RESIDCOM = 'VALIDO' THEN 1 ELSE 0
