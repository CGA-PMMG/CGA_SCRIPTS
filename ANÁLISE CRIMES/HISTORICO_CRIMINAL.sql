/*------------------------------------------------------------------------------------------------------------------------------
 * O script SQL tem como objetivo identificar ocorrências anteriores relacionadas a vítimas, autores, coautores e suspeitos 
 * envolvidos em crimes de homicídio registrados no estado de Minas Gerais no ano de 2024. Ele utiliza uma abordagem estruturada 
 * com uma CTE (Common Table Expression) para isolar os registros de homicídios e, posteriormente, cruza esses dados com 
 * informações históricas de ocorrências anteriores onde o envolvido foi autor, co-autor, suspeito ou vitima.
 * O foco é fornecer um panorama detalhado para subsidiar análises estratégicas e operacionais, com destaque para perfilamento 
 * e rastreamento de envolvidos.
 ------------------------------------------------------------------------------------------------------------------------------*/
...
(
	SELECT 
		...
		ENV.envolvimento_descricao, -- Tipo de envolvimento do indivíduo na ocorrência (ex.: suspeito, vítima)
		ENV.ind_militar_policial, -- Indicador se o envolvido é policial militar
		ENV.digitador_id_orgao, -- ID do órgão que registrou a ocorrência -- 0 PM , 1 PC
		ENV.tipo_prisao_apreensao_descricao, -- Descrição do tipo de prisão ou apreensão realizada
...
CASE WHEN OCO.codigo_municipio in (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		ELSE 'OUTROS' 
	END AS UEOP_2024,
	CAST(OCO.codigo_municipio AS integer) codigo_municipio, -- Conversão do código do município para inteiro
	OCO.nome_municipio, -- Nome do município da ocorrência 
	OCO.nome_bairro, -- Nome do bairro da ocorrência
	OCO.descricao_endereco, -- Descrição do endereço da ocorrência
	OCO.numero_endereco, -- Número do endereçoda da ocorrência
	OCO.unidade_area_militar_codigo, -- Código da unidade área militar 
	OCO.unidade_area_militar_nome, -- Nome da unidade área militar 
	...
	OCO.digitador_sigla_orgao -- Sigla do órgão responsável pela digitação do registro
FROM 
	...
	INNER JOIN HOMICIDIO HM ON (UPPER(ENV.nome_completo_envolvido) = UPPER(HM.nome_completo_envolvido)
								...
								AND CAST(ENV.data_nascimento  AS DATE) = CAST(HM.data_nascimento AS DATE)) -- Junta com a CTE HOMICIDIO com base nos dados do envolvido
WHERE 1=1 -- Filtro genérico para adicionar condições subsequentes
	AND OCO.data_hora_fato BETWEEN '2021-01-01 00:00:00' AND '2024-01-01 00:00:00' -- Filtra data/hora fato dentro do intervalo especificado.
	AND OCO.ind_estado = 'F' -- Filtra ocorrências finalizadas
	AND OCO.ocorrencia_uf = 'MG' -- Filtra ocorrências do estado de Minas Gerais
	AND OCO.nome_tipo_relatorio IN ('POLICIAL','REFAP') -- Filtra relatórios dos tipos "POLICIAL" ou "REFAP"
	AND OCO.digitador_id_orgao IN (0,1) -- Filtra registros de órgãos específicos (0 PM, 1 PC)
	AND ENV.id_envolvimento IN (25,32,1097,26,27,872,28,44,36,35) -- Filtra tipos de envolvimento específicos (TODOS VITIMAS, AUTOR, CO-AUTOR, SUSPEITO)
   -- AND OCO.unidade_area_militar_nome LIKE '%X BPM/X RPM%' -- Filtre a unidade de área militar BPM/RPM
   -- AND OCO.nome_municipio  LIKE '%xxxxxx%'-- Filtre o município
