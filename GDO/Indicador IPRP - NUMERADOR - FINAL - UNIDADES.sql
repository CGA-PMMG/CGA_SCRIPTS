/*----------------------------------------------------------------------------------------------------------------------------------
 * Este script foi desenvolvido para análise do indicador da GDO, IPRP (Indicador de Prisão de Reincidentes Policiais) - NUMERADOR,
 * das unidades. Versão 10/04/2024
 ----------------------------------------------------------------------------------------------------------------------------------*/

WITH 
TODOS_PRESOS AS 
( ------  ESTA CTE BUSCA TODOS OS PRESOS A PARTIR DE 2022------
    SELECT 
        DISTINCT
        UPPER(ENV.nome_completo_envolvido) as NOME_COMPLETO_ENVOLVIDO, -- SELECIONA O NOME COMPLETO DO ENVOLVIDO EM LETRA MAIÚSCULA
        UPPER(ENV.nome_mae) AS NOME_MAE, -- SELECIONA O NOME DA MÃE DO ENVOLVIDO EM LETRA MAIÚSCULA
    ...
    WHERE 1 = 1
    AND BO.digitador_sigla_orgao in ('PM','PC') -- FILTRA O ORGÃO DO DIGITADOR, PM E PC
    AND ENV.id_tipo_prisao_apreensao IN (1,2,3,4,6,7)-- FILTRA O TIPO DE PRISÃO/APREENSÃO : FLAGRANTES(REDS E TCO),MANDADO, RECAPTURA, OUTROS
    AND ENV.data_hora_fato >= '2022-01-01' -- FILTRA A DATA/HORA A PARTIR DE 2022
)
----- ESTA SELEÇÃO RETORNA TODOS OS PRESOS DENTRO DO INDICADOR DO IPRP(NUMERADOR) (GDO) ----
SELECT 
    DISTINCT
    ...
    ENV.numero_cpf_cnpj, -- SELECIONA O NÚMERO DO CPF/CNPJ DO ENVOLVIDO
    ENV.data_hora_fato, -- SELECIONA A DATA/HORA DO FATO
    MONTH (ENV.data_hora_fato) AS mes_fato, -- SELECIONA O MÊS DA DATA/HORA FATO
    cast(ENV.data_nascimento as date) as 'data_nascimento',  -- SELECIONA E CONVERTE A DATA DE NASCIMENTO DO ENVOLVIDO EM DATA
    ENV.natureza_ocorrencia_codigo COD_NATUREZA_ENVOLVIDO, -- SELECIONA O CÓDIGO DA NATUREZA DO ENVOLVIDO
    ENV.natureza_ocorrencia_descricao DESC_NATUREZA_ENVOLVIDO, -- SELECIONA A DESCRIÇÃO DA NATUREZA DO ENVOLVIDO
    ENV.ind_consumado AS CONSUMADO_NATUREZA_ENVOLVIDO,  -- SELECIONA O INDICATIVO DE NATUREZA CONSUMADA DO ENVOLVIDO
    BO.digitador_sigla_orgao, -- SELECIONA A SIGLA DO ORGÃO DO DIGITADOR
    BO.natureza_codigo AS COD_NATUREZA_BO, -- SELECIONA O CÓDIGO DA NATUREZA DA OCORRÊNCIA 
    BO.natureza_descricao AS DESC_NATUREZA_BO, -- SELECIONA A DESCRIÇÃO DA NATUREZA DA OCORRÊNCIA
    BO.natureza_consumado AS CONSUMADO_NATUREZA_BO,  -- SELECIONA O INDICATIVO DE NATUREZA CONSUMADA DO ENVOLVIDO
    cast(BO.codigo_municipio as integer) as codigo_municipio, -- SELECIONA E CONVERTE O CÓDIGO DE MUNICIPIO PARA INTEIRO
    BO.nome_municipio, -- SELECIONA O NOME DO MUNICIPIO
    BO.unidade_area_militar_codigo, -- SELECIONA O CÓDIGO DA UNIDADE DA ÁREA MILITAR
    BO.unidade_area_militar_nome, -- SELECIONA O NOME DA UNIDADE DA  ÁREA MILITAR
    ...
    CASE WHEN BO.codigo_municipio IN (310620) THEN '01 RPM'
   	...
   	END AS RPM_2024,
   	CASE WHEN BO.codigo_municipio in (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		WHEN BO.codigo_municipio in (312770,312690,314010,317150,316160,316300,310180,311265,312370,312580,313320,314995,316770,316840,310220) THEN '06 BPM'
		...
		ELSE 'OUTROS' 
	END AS UEOP_2024	  			
FROM db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV -- TABELA DA ENVOLVIDO
INNER JOIN TODOS_PRESOS T ON 
(
    (
        (
...
INNER JOIN db_bisp_reds_reporting.tb_ocorrencia BO ON BO.numero_ocorrencia = ENV.numero_ocorrencia -- JUNÇÃO COM A TABELA DE OCORRÊNCIA COM A CHAVE:  NÚMERO DA OCORRÊNCIA
WHERE 1 = 1 -- CONDIÇÃO SEMPRE VERDADEIRA PARA FACILITAR A ADIÇÃO DE OUTRAS CONDIÇÕES
	AND BO.digitador_sigla_orgao IN ('PM') -- FILTRA ÓRGÃO DIGITADOR PM 
	AND BO.ind_estado IN ('F') -- FILTRA OCORRÊNCIAS COM ESTADO FECHADO
	AND BO.ocorrencia_uf  IN  ('MG') -- FILTRA UF DA OCORRÊNCIA MG
	AND YEAR(BO.data_hora_fato) = :ANO -- PROMPT PARA FILTRAR PELO ANO DA DATA/HORA DO FATO
	AND MONTH (BO.data_hora_fato) >= :MESINICIAL -- PROMPT PARA FILTRAR PELO MÊS INICIAL DA DATA/HORA DO FATO
    AND MONTH (BO.data_hora_fato) <= :MESFINAL -- PROMPT PARA FILTRAR PELO MÊS FINAL DA DATA/HORA DO FATO
    AND ENV.id_tipo_prisao_apreensao IN (1,2,3,4,6,7) -- FILTRA O TIPO DE PRISÃO/APREENSÃO : FLAGRANTES(REDS E TCO),MANDADO, RECAPTURA, OUTROS
    AND ENV.natureza_ocorrencia_codigo IN ('B01121','B01129','C01157','C01180') -- FILTRA NATUREZAS DE OCORRÊNCIA: HÓMICIDIO, LESÃO CORPORAL, ROUBO, RECEPTAÇÃO
