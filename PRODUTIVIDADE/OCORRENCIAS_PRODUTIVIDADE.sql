...
SELECT OCO.numero_ocorrencia,  -- Número identificador da ocorrência
    OCO.descricao_estado,  -- Estado da ocorrência (ex: Fechado, Aberto)
    OCO.nome_tipo_relatorio,  -- Tipo de relatório (ex: Policial, REFAP, Trânsito)
    OCO.data_hora_fechamento,  -- Data e hora do fechamento da ocorrência
...
END AS situacao_zona,      -- se o território é Urbano ou Rural segundo o IBGE      
    ...
    OCO.sqtempo_fato,  -- Sequencial de tempo do fato
    OCO.ind_estado,  -- Indicador de estado da ocorrência
    OCO.descricao_tipo_relatorio,  -- Descrição do tipo de relatório
    OCO.data_hora_comunicacao,  -- Data e hora quando o fato foi comunicado
    OCO.data_hora_inclusao,  -- Data e hora da inclusão do registro no sistema
    OCO.data_hora_alteracao,  -- Data e hora da última alteração do registro
    OCO.operacao_codigo,  -- Código da operação associada
    OCO.operacao_descricao,  -- Descrição da operação  
    OCO.tipo_local_codigo,  -- Código do tipo de local
    OCO.modo_acao_criminosa,  -- Modo de ação criminosa descrito
    OCO.descricao_endereco,  -- Descrição completa do endereço
    OCO.relator_nome,  -- Nome do relator do registro
    OCO.relator_cargo,  -- Cargo do relator
    OCO.ind_tco,  -- Indicador se foi lavrado um Termo Circunstanciado de Ocorrência
    OCO.data_hora_final,  -- Data e hora final registrada para o fato
    OCO.numero_atividade_cad,  -- Número da atividade no sistema CAD
    OCO.numero_chamada_cad,  -- Número da chamada no sistema CAD
    OCO.numero_chamada_principal_cad,  -- Número da chamada principal no sistema CAD
    OCO.digitador_cargo_efetivo,  -- Cargo efetivo do digitador
    OCO.digitador_nome  -- Nome do digitador
...
WHERE 1 = 1
    AND OCO.data_hora_fato BETWEEN '2025-01-01 00:00:00.000' AND '2025-02-28 23:59:59.000' -- Filtra ocorrências por período específico 
