/* -------------------------------------------------------------------------------------------------------------------------------------------------------
 *  ================================================================================================================================================
 *  ====================================== 2.4 REVITIMIZAÇÃO APÓS ENCERRAMENTO DO PROTOCOLO DE SEGUNDA RESPOSTA (RPSR) =============================
 *  ================================================================================================================================================
 * 
 * O Indicador tem por finalidade avaliar a reincidência de violência doméstica (mesmo autor e mesma vítima) após o encerramento do
...
WITH VITIMAS AS (                                                      -- CTE que identifica vítimas em ocorrências após possível revitimização
    SELECT 
        ...
        OCO.data_hora_fato,                                           -- Data e hora do fato ocorrido
...
ELSE 'OUTRAS' END AS CIA_PVD
    FROM db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV           -- Tabela contendo informações dos envolvidos nas ocorrências
    JOIN db_bisp_reds_reporting.tb_ocorrencia OCO                     -- Tabela principal de registros de ocorrência
        ON ENV.numero_ocorrencia = OCO.numero_ocorrencia              -- Junção baseada no número da ocorrência
    WHERE 
        ENV.id_envolvimento IN (25, 26, 27, 28, 32, 872, 1097)        -- Filtro para garantir que o envolvido seja a vítima
        AND (ENV.codigo_sexo = 'F' OR identidade_genero_codigo IN ('0400', '0200', '0700', '0100', '0600'))  --Sexo feminino Ou identidade de gênero correspondente
...
CASOS_ENCERRAMENTOS AS (         -- CTE que identifica ocorrências que indicam o encerramento do protocolo de segunda resposta
    SELECT 
        ...
        OCO.data_hora_fato AS data_visita,                            -- Data e hora da ocorrência de encerramento
...
SELECT  
    ...
    V.natureza_codigo,                                                -- Código da natureza da ocorrência de revitimização
    V.data_hora_fato,                                                 -- Data e hora do fato de revitimização
    C.registro_visita,                                                -- Número da ocorrência do encerramento do Protocolo de Segunda Resposta
    C.natureza_visita,                                                -- Natureza do encerramento do Protocolo de Segunda Resposta
    C.data_visita,                                                    -- Data em que foi feito o encerramento do Protocolo de Segunda Resposta
    V.natureza_secundaria1_codigo,                                    -- Códigos das primeira natureza secundária da revitimização
    V.natureza_secundaria2_codigo,									  -- Códigos das segunda natureza secundária da revitimização
    V.natureza_secundaria3_codigo,									  -- Códigos das terceira natureza secundária da revitimização
    V.nome_municipio,                                                 -- Município da revitimização
    V.codigo_municipio,                                               -- Código do município da revitimização
    V.RPM_2025,                                                       -- RPM 2025
